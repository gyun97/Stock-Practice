        ...
        public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
                return http
                                ...
                                .sessionManagement(session -> session
                                                .sessionCreationPolicy(SessionCreationPolicy.STATELESS) // 세션을 생성하거나
                                                                                                        // 저장하지 않음(모든
                                                                                                        // 요청은 오직 JWT 토큰
                                                                                                        // 기반으로 인증)
                                )
                                ...
                                .anonymous(org.springframework.security.config.Customizer.withDefaults()) // 익명 사용자 허용
                                                                                                          // (permitAll
                                                                                                          // 설정에
                                                                                                          // 필요)
                                .httpBasic(AbstractHttpConfigurer::disable) // Authorization 헤더로 Base64 로그인 비활성(JWT 사용 시
                                                                            // 중복)
